...
def test_l3_basic_consistency(worker_script):
    """Case 1: consistency (NP=1 vs NP=4)"""
    total_envs = 16
    steps = 10
...
def test_l3_remainder_distribution(worker_script):
    """Case 2: remainder (10 envs / 3 ranks)"""
    total_envs = 10
    np = 3
    steps = 5
...
def test_l3_oversubscription(worker_script):
    """Case 3: over allocation(2 envs / 4 ranks)"""
    total_envs = 2
    np = 4
    steps = 5
...
def test_l3_heavy_load(worker_script):
    """Case 4: stress test (64 envs / 4 ranks)"""
    total_envs = 64
    np = 4
    steps = 50
